...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // 处理点击事件
  document.getElementById('initialChart').addEventListener('click', function (evt) {
    ...
    if (points.length) {
      ...
      if (label === '抑郁') {
        console.log('Segmenting the "抑郁" part');

        initialChart.data.labels = ['未抑郁', '轻度抑郁', '中度抑郁', '重度抑郁'];
        initialChart.data.datasets[0].data = [39.4, 25.725, 25.725, 9.15];
        initialChart.data.datasets[0].backgroundColor = ['#ffffe0', '#ff95a4', '#a84759', '#4f0013'];
        initialChart.update();
  ...
  document.getElementById('initialChart').addEventListener('mouseleave', function () {
    ...
    if (isDetailChartVisible) {
      console.log('Reverting the initial chart to original state');
      initialChart.data.labels = ['抑郁', '未抑郁'];
      initialChart.data.datasets[0].data = [60.6, 39.4];
      initialChart.data.datasets[0].backgroundColor = ['#6890A2', '#ffffe0'];
      initialChart.update();
